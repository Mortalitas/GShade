...
float3 getEyePos(coords)
{
	float EyeDepth = GetProDepth(coords.xy);
	return float3((coords.xy*2.0-1.0), EyeDepth);
}

float3 GetBumpNormals ( in sampler2D colorsampler, texCoord) {
...
float3 GetScreenNormalsAMD(tex)
{
...
void PS_RBM_Setup(vpos : SV_Position, texcoord : TEXCOORD, out hdrR : SV_Target0)
{
...
float4 PS_RBM_Execute(vpos : SV_Position, texcoord : TEXCOORD) : SV_Target
{
...
technique RBM_Tech <enabled = RESHADE_START_ENABLED; toggle = RBM_ToggleKey; >
{
	pass RBMSetupPass
	{
