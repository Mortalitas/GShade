/*=============================================================================
    ...
    Ambient Obscurance with Indirect Lighting "GTAO"
...
#ifndef GTAO_MIPLEVEL_AO
 #define GTAO_MIPLEVEL_AO		0	//[0 to 2]      Miplevel of AO texture. 0 = fullscreen, 1 = 1/2 screen width/height, 2 = 1/4 screen width/height and so forth. Best results: IL MipLevel = AO MipLevel + 2
#endif

#ifndef GTAO_MIPLEVEL_IL
 #define GTAO_MIPLEVEL_IL		2	//[0 to 4]      Miplevel of IL texture. 0 = fullscreen, 1 = 1/2 screen width/height, 2 = 1/4 screen width/height and so forth.
#endif

#ifndef GTAO_ENABLE_IL
 #define GTAO_ENABLE_IL			0	//[0 or 1]	    Enables Indirect Lighting calculation. Will cause a major fps hit.
#endif

#ifndef GTAO_SMOOTHNORMALS
 #define GTAO_SMOOTHNORMALS     1   //[0 or 1]      This feature makes low poly surfaces smoother, especially useful on older games.
#endif

#ifndef GTAO_TWO_LAYER
 #define GTAO_TWO_LAYER         0   //[0 or 1]      Splits GTAO into two separate layers that allow for both large and fine AO.
#endif

#ifndef GTAO_HQ
 #define GTAO_HQ                1   //[0 or 1]      Enables a different, more physically accurate but slower SSAO mode. Based on Ground Truth Ambient Occlusion by Activision. No IL yet.
#endif
...
uniform int GTAO_GLOBAL_SAMPLE_QUALITY_PRESET <
	ui_type = "combo";
...
uniform float GTAO_SAMPLE_RADIUS <
	ui_type = "slider";
	ui_min = 0.5; ui_max = 20.0;
    ui_label = "Sample Radius";
	ui_tooltip = "Sample radius of GTAO, higher means more large-scale occlusion with less fine-scale details.";  
    ui_category = "Global";      
> = 2.5;

#if (GTAO_HQ==0)
    uniform float GTAO_SAMPLE_NORMAL_BIAS <
        ui_type = "slider";
    ...
    > = 0.2;
#else
    #define GTAO_SAMPLE_NORMAL_BIAS 0       //don't break PS which needs this, cleaner this way
#endif

uniform float GTAO_GLOBAL_RENDER_SCALE <
	ui_type = "slider";
    ui_label = "Render Size Scale";
	ui_min = 0.50; ui_max = 1.00;
    ui_tooltip = "Factor of GTAO resolution, lower values greatly reduce performance overhead but decrease quality.\n1.0 = GTAO is computed in original resolution\n0.5 = GTAO is computed in 1/2 width 1/2 height of original resolution\n...";
    ui_category = "Global";
> = 1.0;

uniform float GTAO_SSAO_AMOUNT <
	ui_type = "slider";
...
#if(GTAO_ENABLE_IL != 0)
uniform float GTAO_SSIL_AMOUNT <
    ui_type = "slider";
...
uniform float GTAO_SSIL_SATURATION <
    ui_type = "slider";
...
> = 1.00;
...
#if (GTAO_TWO_LAYER != 0)
    uniform float GTAO_SAMPLE_RADIUS_SECONDARY <
        ui_type = "slider";
    ...
    uniform float GTAO_AMOUNT_FINE <
        ui_type = "slider";
    ...
    uniform float GTAO_AMOUNT_COARSE <
        ui_type = "slider";
        ui_min = 0.00; ui_max = 1.00;
...
uniform float GTAO_FADE_DEPTH_START <
	ui_type = "slider";
    ui_label = "Fade Out Start";
	ui_min = 0.00; ui_max = 1.00;
	ui_tooltip = "Distance where GTAO starts to fade out. 0.0 = camera, 1.0 = sky. Must be less than Fade Out End.";
    ui_category = "Blending";
> = 0.05;

uniform float GTAO_FADE_DEPTH_END <
	ui_type = "slider";
    ui_label = "Fade Out End";
	ui_min = 0.00; ui_max = 1.00;
	ui_tooltip = "Distance where GTAO completely fades out. 0.0 = camera, 1.0 = sky. Must be greater than Fade Out Start.";
    ui_category = "Blending";
> = 0.4;

uniform int GTAO_DEBUG_VIEW_ENABLE <
	ui_type = "combo";
...
texture2D GTAO_ColorTex 	{ Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = RGBA8; MipLevels = 3+GTAO_MIPLEVEL_IL;};
texture2D GTAO_DepthTex 	{ Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = R16F;  MipLevels = 3+GTAO_MIPLEVEL_AO;};
texture2D GTAO_NormalTex	{ Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = RGBA8; MipLevels = 3+GTAO_MIPLEVEL_IL;};

sampler2D sGTAO_ColorTex	{ Texture = GTAO_ColorTex;	};
sampler2D sGTAO_DepthTex	{ Texture = GTAO_DepthTex;	};
sampler2D sGTAO_NormalTex	{ Texture = GTAO_NormalTex;	};
...
GTAO_VSOUT VS_GTAO(in uint id : SV_VertexID)
{
    GTAO_VSOUT MXAO;

    MXAO.uv.x = (id == 2) ? 2.0 : 0.0;
    MXAO.uv.y = (id == 1) ? 2.0 : 0.0;
    MXAO.uv.zw = MXAO.uv.xy / GTAO_GLOBAL_RENDER_SCALE;
    ...
    MXAO.samples   = samples_per_preset[GTAO_GLOBAL_SAMPLE_QUALITY_PRESET];
...
float3 get_position_from_uv(in float2 uv, in GTAO_VSOUT MXAO)
{
    return (uv.xyx * MXAO.uvtoviewMUL + MXAO.uvtoviewADD) * qUINT::linear_depth(uv) * RESHADE_DEPTH_LINEARIZATION_FAR_PLANE;
}

float3 get_position_from_uv_mipmapped(in float2 uv, in GTAO_VSOUT MXAO, in int miplevel)
{
    return (uv.xyx * MXAO.uvtoviewMUL + MXAO.uvtoviewADD) * tex2Dlod(sGTAO_DepthTex, float4(uv.xyx, miplevel)).x;
}
...
void spatial_blur_data(inout BlurData o, in sampler inputsampler, in float inputscale, in float4 uv)
{
	...
	o.mask = tex2Dlod(sGTAO_NormalTex, uv);
...
float4 blur_filter(in GTAO_VSOUT MXAO, in sampler inputsampler, in float inputscale, in float radius, in int blursteps)
{
...
void sample_parameter_setup(in GTAO_VSOUT MXAO, in float scaled_depth, in float layer_id, out float scaled_radius, out float falloff_factor)
{
    scaled_radius  = 0.25 * GTAO_SAMPLE_RADIUS / (MXAO.samples * (scaled_depth + 2.0));
    falloff_factor = -1.0/(GTAO_SAMPLE_RADIUS * GTAO_SAMPLE_RADIUS);

    #if(GTAO_TWO_LAYER != 0)
        scaled_radius  *= lerp(1.0, GTAO_SAMPLE_RADIUS_SECONDARY + 1e-6, layer_id);
        falloff_factor *= lerp(1.0, 1.0 / (GTAO_SAMPLE_RADIUS_SECONDARY * GTAO_SAMPLE_RADIUS_SECONDARY + 1e-6), layer_id);
    #endif
}

void smooth_normals(inout float3 normal, in float3 position, in GTAO_VSOUT MXAO)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        for(int direction_step = 1; direction_step <= 5; direction_step++)
        {
            ...
            float3 temp_normal = tex2Dlod(sGTAO_NormalTex, float4(sample_uv, 0, 0)).xyz * 2.0 - 1.0;
...
void PS_InputBufferSetup(in GTAO_VSOUT MXAO, out float4 color : SV_Target0, out float4 depth : SV_Target1, out float4 normal : SV_Target2)
{
...
void PS_StencilSetup(in GTAO_VSOUT MXAO, out float4 color : SV_Target0)
{        
    if(    qUINT::linear_depth(MXAO.uv.zw) >= GTAO_FADE_DEPTH_END
        || 0.25 * 0.5 * GTAO_SAMPLE_RADIUS / (tex2D(sGTAO_DepthTex, MXAO.uv.zw).x + 2.0) * BUFFER_HEIGHT < 1.0
        || MXAO.uv.z > 1.0
        || MXAO.uv.w > 1.0
        ) discard;
...
void PS_AmbientObscurance(in GTAO_VSOUT MXAO, out float4 color : SV_Target0)
{
    ...
    if(GTAO_GLOBAL_SAMPLE_QUALITY_PRESET == 7) MXAO.samples = 2 + floor(0.05 * GTAO_SAMPLE_RADIUS / linear_depth);
    ...
    {                    
    	...
    	float3 occlusion_vector = -position + get_position_from_uv_mipmapped(sample_uv, MXAO, sample_mip + GTAO_MIPLEVEL_AO);                
        ...
        float sample_occlusion = saturate(1.0 + falloff_factor * occlusion_distance_squared) * saturate(occlusion_normal_angle - GTAO_SAMPLE_NORMAL_BIAS);
#if(GTAO_ENABLE_IL != 0)
        [branch]
        if(sample_occlusion > 0.1)
        {
                float3 sample_indirect_lighting = tex2Dlod(sGTAO_ColorTex, float4(sample_uv, 0, sample_mip + GTAO_MIPLEVEL_IL)).xyz;
                float3 sample_normal = tex2Dlod(sGTAO_NormalTex, float4(sample_uv, 0, sample_mip + GTAO_MIPLEVEL_IL)).xyz * 2.0 - 1.0;
    ...
    color = saturate(color / ((1.0 - GTAO_SAMPLE_NORMAL_BIAS) * MXAO.samples) * 2.0);
    color = color.BLUR_COMP_SWIZZLE;

#if(GTAO_TWO_LAYER != 0)
    color *= lerp(GTAO_AMOUNT_COARSE, GTAO_AMOUNT_FINE, layer_id); 
#endif
}

void PS_AmbientObscuranceHQ(in GTAO_VSOUT MXAO, out float4 color : SV_Target0)
{
	float3 position = get_position_from_uv_mipmapped(MXAO.uv.zw, MXAO, 0);
	float3 normal 	= tex2D(sGTAO_NormalTex, MXAO.uv.zw).xyz * 2.0 - 1.0;	
	...
	float scaled_radius = GTAO_SAMPLE_RADIUS / position.z / stepshalf * RESHADE_DEPTH_LINEARIZATION_FAR_PLANE;
	float falloff_factor = 0.25 * rcp(GTAO_SAMPLE_RADIUS * GTAO_SAMPLE_RADIUS);	
	...
	for(float i = 0; i < directions; i++)
	{
		...
		for(int j = 0; j < stepshalf; j++)
		{
			...
			occlusion_vector[0] = -position + get_position_from_uv_mipmapped(sample_uv.xy, MXAO, sample_mip + GTAO_MIPLEVEL_AO);  
			occlusion_vector[1] = -position + get_position_from_uv_mipmapped(sample_uv.zw, MXAO, sample_mip + GTAO_MIPLEVEL_AO); 
...
void PS_SpatialFilter1(in GTAO_VSOUT MXAO, out float4 color : SV_Target0)
{
    color = blur_filter(MXAO, qUINT::sCommonTex0, GTAO_GLOBAL_RENDER_SCALE, 0.75, 4);
}

void PS_SpatialFilter2(GTAO_VSOUT MXAO, out float4 color : SV_Target0)
{
    float4 ssil_ssao = blur_filter(MXAO, qUINT::sCommonTex1, 1, 1.0 / GTAO_GLOBAL_RENDER_SCALE, 8);

	color = tex2D(sGTAO_ColorTex, MXAO.uv.xy);
...
#if(GTAO_ENABLE_IL != 0)
	ssil_ssao.xyz  = lerp(dot(ssil_ssao.xyz, lumcoeff), ssil_ssao.xyz, GTAO_SSIL_SATURATION) * GTAO_SSIL_AMOUNT * 2.0;
#else
    ssil_ssao.xyz = 0.0;
#endif
...
#if(GTAO_HQ == 0)
	ssil_ssao.w  = 1.0 - pow(1.0 - ssil_ssao.w, GTAO_SSAO_AMOUNT * 2.0);
#else
    ssil_ssao.w  = 1.0 - pow(1.0 - ssil_ssao.w, GTAO_SSAO_AMOUNT);
#endif
    ssil_ssao    *= 1.0 - smoothstep(GTAO_FADE_DEPTH_START, GTAO_FADE_DEPTH_END, scenedepth * float4(2.0, 2.0, 2.0, 1.0));

    if(GTAO_BLEND_TYPE == 0)
    {
        color.rgb -= (ssil_ssao.www - ssil_ssao.xyz) * blendfact * color.rgb;
    }
    else if(GTAO_BLEND_TYPE == 1)
    {
        color.rgb = color.rgb * saturate(1.0 - ssil_ssao.www * blendfact * 1.2) + ssil_ssao.xyz * blendfact * colorgray * 2.0;
    }
    else if(GTAO_BLEND_TYPE == 2)
    {
    ...
    if(GTAO_DEBUG_VIEW_ENABLE == 1)
    {
        color.rgb = max(0.0, 1.0 - ssil_ssao.www + ssil_ssao.xyz);
        color.rgb *= (GTAO_ENABLE_IL != 0) ? 0.5 : 1.0;
    }
    else if(GTAO_DEBUG_VIEW_ENABLE == 2)
    {      
        color.rgb = tex2D(sGTAO_NormalTex, MXAO.uv.xy).xyz;
...
technique qGTAO 
< ui_tooltip = "                     >> qUINT::GTAO <<\n\n"
			   "GTAO is a screen-space ambient occlusion shader.\n"
               ...
               "\nMake sure to move GTAO to the very top of your shader list for\n"
               "maximum compatibility with other shaders.\n"
               "\nGTAO is written by Marty McFly / Pascal Gilcher"; >
{
    pass
	{
		VertexShader = VS_GTAO;
		PixelShader  = PS_InputBufferSetup;
		RenderTarget0 = GTAO_ColorTex;
		RenderTarget1 = GTAO_DepthTex;
		RenderTarget2 = GTAO_NormalTex;
	}
    pass
    {
        VertexShader = VS_GTAO;
