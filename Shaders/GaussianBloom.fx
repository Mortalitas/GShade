...
if(GaussianBloomRadius == 0)	
{
	...
	for(int i = 1; i < 4; ++i)
	{
		blur += tex2Dlod(GaussianBloomSampler, float4(texcoord + float2(0.0, offset[i] * BUFFER_PIXEL_SIZE.y), 0.0, 0.0)).rgb * weight[i];
		blur += tex2Dlod(GaussianBloomSampler, float4(texcoord - float2(0.0, offset[i] * BUFFER_PIXEL_SIZE.y), 0.0, 0.0)).rgb * weight[i];
	}
}	
...
if(GaussianBloomRadius == 1)	
{
	...
	for(int i = 1; i < 6; ++i)
	{
		blur += tex2Dlod(GaussianBloomSampler2, float4(texcoord + float2( offset[i] * BUFFER_PIXEL_SIZE.x,0.0), 0.0, 0.0)).rgb * weight[i];
		blur += tex2Dlod(GaussianBloomSampler2, float4(texcoord - float2( offset[i] * BUFFER_PIXEL_SIZE.x,0.0), 0.0, 0.0)).rgb * weight[i];
	}
}	
