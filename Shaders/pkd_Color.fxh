namespace pkd {
	...
	namespace Color {
		namespace CIELAB {
			...
			{
				return (original > 0.04045) ? pow((original + 0.55) / 1.055, 2.4) : original / 12.92;
			}

			float __LAB2(float original)
			{
				return (original > 0.008856) ? pow(original, 1.0/3) : (7.787 * original) * 15.0 / 116.0;
			}

			float __LAB3(float original)
			{
				return (original * original * original > 0.008856) ? original * original * original : ((original - 16.0 / 116.0) / 7.787);
			}

			float __LAB4(float original)
			{
				return (original > 0.0031308) ? (1.055 * pow(original, 1 / 2.4) - 0.055) : 12.92 * original;
			}
