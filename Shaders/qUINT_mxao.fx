...
void PS_qAmbientObscuranceHQ(in qMXAO_VSOUT MXAO, out float4 color : SV_Target0)
{
	...
	for(float i = 0; i < directions; i++)
	{
		...
		for(int j = 0; j < stepshalf; j++)
		{
			...
			if(falloff.x < 0.8)
			{
				sample_il[0] = tex2Dlod(sMXAO_ColorTex, float4(sample_uv.xy, 0, sample_mip + qMXAO_MIPLEVEL_IL)).xyz;
				sample_normal[0] = tex2Dlod(sMXAO_NormalTex, float4(sample_uv.xy, 0, sample_mip + qMXAO_MIPLEVEL_IL)).xyz * 2.0 - 1.0;
			...
			if(falloff.y < 0.8)
			{
	            sample_il[1] = tex2Dlod(sMXAO_ColorTex, float4(sample_uv.zw, 0, sample_mip + qMXAO_MIPLEVEL_IL)).xyz;
	            sample_normal[1] = tex2Dlod(sMXAO_NormalTex, float4(sample_uv.zw, 0, sample_mip + qMXAO_MIPLEVEL_IL)).xyz * 2.0 - 1.0;       
