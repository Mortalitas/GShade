...
float3 MadCakeToneMapPass(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	if (DeGamma)
	{
		color.rgb = pow(abs(color.rgb), 0.45454545);
	}
	...
	color.r = pow(abs(color.r), Contrast - (Contrast - 1.0) * color.r);
	color.g = pow(abs(color.g), Contrast - (Contrast - 1.0) * color.g);
	color.b = pow(abs(color.b), Contrast - (Contrast - 1.0) * color.b);
