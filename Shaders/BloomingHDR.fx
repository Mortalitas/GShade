...
float Luma(float3 C)
{
	if (HDR_Toggle == 0)
	{
		const float3 Luma = float3(0.2126, 0.7152, 0.0722); // (HD video) https://en.wikipedia.org/wiki/Luma_(video)
		return dot(C,Luma);
	}
	else
	{
		const float3 Luma = float3(0.2627, 0.6780, 0.0593); // (HDR video) https://en.wikipedia.org/wiki/Rec._2100
		return dot(C,Luma);
	}
}
...
float CalcExposedColor(in float avgLuminance, in float offset, in float GreyValue )
	{
	    const float exposure = Log2Exposure( avgLuminance, GreyValue  * 2.2 ) + offset; //offset = exposure
...
float3 Downsample(float4 pos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
	...
	const float2 midHV = (Adapt_Seek-1) * float2(BUFFER_WIDTH * 0.5,BUFFER_HEIGHT * 0.5) * pix;
...
{   //Temporal adaptation https://knarkowicz.wordpress.com/2016/01/09/automatic-exposure/
	...
	const float ITF= 1-(smoothstep(1,0,TT) < 0.85);// Is trigger Based on a Treshold... //smoothstep(1,0,tex2D(SamplerTA,0.0).y) < 0.85
	const float FT = 2500, PStoredfade = tex2D(SamplerStored, float2(0.75,0.25)).x;//Top Right
	const float TF = PStoredfade + (ITF - PStoredfade) * (1.0 - exp2(-frametime/FT)); //Fade Time
	//float3(EA,TT,TF)
...
{
    const float hdrMax =  HDR_Toggle ? 25.0 : 16.0; // How much HDR range before clipping. HDR modes likely need this pushed up to say 25.0.
    const float contrast = Contrast + 0.250; // Use as a baseline to tune the amount of contrast the tonemapper has.
    ...
    static const float midIn = 0.11; // most games will have a {0.0 to 1.0} range for LDR so midIn should be 0.18.But,I settled around 0.11
    const float midOut = HDR_Toggle ? 0.18/25.0 : 0.18; // Use for LDR. For HDR10 10:10:10:2 use maybe 0.18/25.0 to start. For scRGB, I forget what a good starting point is, need to re-calculate.
    ...
    const float saturation = contrast * Saturate; // full tonal range saturation control
...
{
	const float PosX = 0.9525f*BUFFER_WIDTH*pix.x,PosY = 0.975f*BUFFER_HEIGHT*pix.y;
