/*-----------------------------------------------------------------------------------------------------*/
/* Slit Scan Shader v1.0 - by Radegast Stravinsky of Ultros.                                               */
...
{
    float4 col_pixels =  tex2D(samplerColor, float2(x, texcoord.y));
    const float pix_w =  scan_speed * 0.005;
...
void SlitScanPost(float4 pos : SV_Position, float2 texcoord : TEXCOORD0, out float4 color : SV_TARGET)
{
    ...
    float pix_w =  scan_speed * 0.005;
    uv.x +=  (anim_rate * pix_w) - x % 1 ;
    float4 buffer = tex2D(ssTarget, uv);


    float4 mask = step(texcoord.x, x);
    color = lerp(screen, depth >= min_depth ? buffer : screen, mask);   
}
