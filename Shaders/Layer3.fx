...
//Second Layer
texture Three_Layer_texture <source="Layer3.png";> { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format=TEXFORMAT; };

sampler Three_Layer_sampler { Texture = Three_Layer_texture; };

float3 Three_PS_Layer(float4 pos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target {
    float3 twocolor = tex2D(ReShade::BackBuffer, texcoord).rgb;
    float4 twolayer = tex2D(Three_Layer_sampler, texcoord).rgba;
    
    twocolor = lerp(twocolor, twolayer.rgb, twolayer.a * Layer_Three_Blend);
