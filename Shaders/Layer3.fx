...
#ifndef Layer3Tex
#define Layer3Tex "LayerStage.png" // Add your own image file to \reshade-shaders\Textures\ and provide the new file name in quotes to change the image displayed!
#endif
...
#define LAYER3_TEXFORMAT RGBA16
#endif

BLENDING_COMBO(Layer3_BlendMode, "Blending Mode", "Select the blending mode applied to the layer.", "", false, 0, 0)
...
texture Layer3_Tex <source = Layer3Tex;> { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format=LAYER3_TEXFORMAT; };
...
void PS_Layer3(in float4 pos : SV_Position, float2 texCoord : TEXCOORD, out float4 passColor : SV_Target) {
    ...
    const float ScaleX =  ScaleSize.x * AspectX * Layer3_ScaleX;
    const float ScaleY =  ScaleSize.y * AspectY * Layer3_ScaleY;
    ...
    passColor = float4(ComHeaders::Blending::Blend(Layer3_BlendMode, backColor.rgb, passColor.rgb, passColor.a * Layer3_Blend), backColor.a);
}
