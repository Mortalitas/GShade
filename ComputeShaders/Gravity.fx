...
namespace alt {
	...
	{
		float real_max_distance = tex2Dfetch(SamplerGravityDistanceMap, float2(vpos.xy), vpos.w).r;
		int iterations = round(real_max_distance * BUFFER_HEIGHT);

		vpos.z = 0;
		float4 sample_pos = vpos;
		for (float depth = tex2Dfetch(SamplerGravityBuf, float2(vpos.xy), vpos.w).x;
			vpos.z < iterations; ++vpos.z, --vpos.y)
		{
			float4 samp = tex2Dfetch(SamplerGravityBuf, float2(vpos.xy), vpos.w);
			samp.w *= samp.z;
		...
		float4 colFragment = tex2Dfetch(ReShade::BackBuffer, float2(sample_pos.xy), sample_pos.w);
		return lerp(colFragment, float4(BlendColor, 1.0), sample_pos.z * EffectFactor);
	}

	float rng_delta()
	{
		float2 old_rng = tex2Dfetch(SamplerGravityCurrentSeed, float2((0).xx), (0).x).x;
